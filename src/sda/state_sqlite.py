...
class SQLiteStateRepository:
    ...
    def create_run(self, run: RunRecord) -> RunRecord:
        existing = self._connection.execute("SELECT run_id FROM runs WHERE idempotency_key = ?", (run.idempotency_key,)).fetchone()
        if existing:
            return self.get_run(existing[0])
        try:
            self._connection.execute(
                "INSERT INTO runs VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (run.run_id, run.request_id, run.idempotency_key, run.status.value, run.plan_id, run.plan_fingerprint, run.version, run.updated_at),
            )
            self._connection.commit()
    ...
    def get_run(self, run_id: str) -> RunRecord:
        row = self._connection.execute("SELECT run_id, request_id, idempotency_key, status, plan_id, plan_fingerprint, version, updated_at FROM runs WHERE run_id = ?", (run_id,)).fetchone()
        if row is None:
            raise StateError(f"unknown run: {run_id}")
        return RunRecord(row[0], row[1], row[2], WorkflowStatus(row[3]), row[4], row[5], row[6], row[7])
    ...
    def transition_run(self, run_id: str, status: WorkflowStatus, *, expected_version: int | None = None) -> RunRecord:
        ...
        params = (updated.status.value, updated.version, updated.updated_at, run_id, current.version)
        changed = self._connection.execute("UPDATE runs SET status = ?, version = ?, updated_at = ? WHERE run_id = ? AND version = ?", params).rowcount
        if changed != 1:
