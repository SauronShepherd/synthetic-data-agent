...
def complex_metrics(values: Iterable[Any], kind: str) -> tuple[dict[str, Any], tuple[str, ...]]:
    ...
    if kind == "struct":
        ...
        def depth(value: Any) -> int:
            ...
            return 0
        element_types = sorted({type(item).__name__ for value in present if isinstance(value, dict) for item in value.values()})
        return (
    ...
    if kind in {"binary", "large_text"}:
        return ({"present_count": len(present), "available": False},
